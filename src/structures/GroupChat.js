...
class GroupChat extends Chat {
  ...
  async removeParticipants(participantIds) {
    ...
    return await this.client.mPage.evaluate(
      async ({ chatId, participantIds }) => {
        return await window.WWebJS.group.removeParticipants(
          chatId,
          participantIds
        );
      },
  ...
  async promoteParticipants(participantIds) {
    ...
    return await this.client.mPage.evaluate(
      async ({ chatId, participantIds }) => {
        return await window.WWebJS.group.promoteParticipants(
          chatId,
          participantIds
        );
      },
  ...
  async demoteParticipants(participantIds) {
    ...
    return await this.client.mPage.evaluate(
      async ({ chatId, participantIds }) => {
        return await window.WWebJS.group.demoteParticipants(
          chatId,
          participantIds
        );
      },
  ...
  async setSubject(subject) {
    const success = await this.client.mPage.evaluate(
  ...
  async setMemberApprovalMode(adminsOnly = true) {
    const succes = await this.client.mPage.evaluate(
      async ({ chatId, adminsOnly }) => {
        const chatWid = window.Store.WidFactory.createWid(chatId);
        try {
          await window.Store.GroupUtils.setGroupProperty(
  ...
  async reportExitClear(type = "AccountInfoReport") {
    await this.client.mPage.evaluate(
      async ({ chatId, type }) => {
        ...
        return await window.Store.GroupUtils.sendSpamExitClear(
          chat,
          SpamFlow[type]
        );
      },
